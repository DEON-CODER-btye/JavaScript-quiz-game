...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (document.querySelector(".questions1")) {
    ...
    function loadQuestion(index) {
      ...
      options.forEach((el, i) => {
        ...
        el.onclick = () => {
          options.forEach((btn) => (btn.style.pointerEvent = "none"));
          if (i === q.correct) {
            el.classList.add("correct");
            correctAnswer++;
          } else {
  ...
  if (document.querySelector('.scores')) {
    const scores = document.querySelector('.scores')
    const greenRate = document.querySelector('.green-rate')
    const greenBox = document.querySelector('.green-rate')
    const redRate = document.querySelector('.green-rate')
    const redBox = document.querySelector('.green-rate')
    const retry = document.querySelector('.retry-btn')

    const corrects = parseInt(localStorage.getItem('correct'))
    const total = parseInt(localStorage.getItem('total'))

    const greenPercent = (corrects / total) * 100;
    scores.textContent = `Your Scored ${corrects}/${total}`;
    greenBox.style.width = `${greenPercent}%`
    greenRate.textContent = `${greenPercent.toFixed(0)}%`
